...
Вместо справочных категорий — своя классификация на основе статистики:

```
Формула:
complexity = 0.4 × norm(avg_time)        // 40% - время обработки
           + 0.3 × norm(std_dev)         // 30% - вариативность
           + 0.2 × norm(typical_qty)     // 20% - типичное количество
           + 0.1 × norm(1/frequency)     // 10% - редкость

Результат: round(complexity × 9) + 1  → от 1 до 10
```

| Категория | Описание | Назначение |
|-----------|----------|------------|
| 1-3 | Лёгкие: быстрые, стабильные, частые | Любой пикер |
| 4-6 | Средние: стандартные товары | Обычное назначение |
| 7-10 | Сложные: долгие, нестабильные, редкие | Опытным пикерам |

**Преимущества:**
- Не зависит от заполненности справочника категорий
- Основано на реальных данных работы
- Автоматически обновляется при переобучении
- Для новых товаров — временно категория 5

**Файлы для реализации:**
- `Layers/Historical/Persistence/ProductStatisticsRepository.cs`
- CLI: `--calc-product-stats`
- Добавить поле `complexity_score` в `product_statistics`
